...
export default function ExperienceTab() {
  ...
  const handleEdit = (item: any) => {
    ...
    setFormData({
      ...item,
      descriptionsStr: item.descriptions?.join('\n') || '',
  ...
  const handleCreate = () => {
    ...
    setFormData({
      ...
      descriptionsStr: '', techStr: '', display_order: items.length
    })
  }
  ...
  const handleSave = async () => {
    try {
      const payload = {
        ...formData,
        descriptions: formData.descriptionsStr.split('\n').filter((s: string) => s.trim()),
        technologies: formData.techStr.split(',').map((s: string) => s.trim()).filter(Boolean)
      }
      if (editingId === 0) {
        await apiFetch('/api/experience', { method: 'POST', body: JSON.stringify(payload) })
      } else {
        await apiFetch(`/api/experience/${editingId}`, { method: 'PUT', body: JSON.stringify(payload) })
      }
      setEditingId(null)
      fetchData()
        ...
        {editingId !== null && formData && (
          <div className="p-4 rounded-xl border space-y-3" style={{ background: 'var(--bg-card)', borderColor: 'var(--accent)' }}>
            <div className="grid grid-cols-2 gap-3">
              {(['year', 'company', 'location', 'position'] as const).map(f => (
                <div key={f}>
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: 'var(--text-muted)' }}>Descriptions (one per line)</label>
              <textarea value={formData.descriptionsStr} onChange={e => setFormData({ ...formData, descriptionsStr: e.target.value })} rows={5}
            ...
            <div>
              ...
              <input value={formData.techStr} onChange={e => setFormData({ ...formData, techStr: e.target.value })}
                className="w-full px-3 py-1.5 rounded border text-sm"
                style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border)', color: 'var(--text-primary)' }} />
            </div>
          ...
          {items.map((item, index) => (
            <div key={item.id}
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold truncate">{item.company}</div>
                <div className="text-xs" style={{ color: 'var(--accent)' }}>{item.position}</div>
                <div className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{item.year}</div>
              </div>

...
export default memo(function ChartBlock({ code }: ChartBlockProps) {
  ...
  if (!parsed) {
    return (
      <pre
        className="my-6 p-4 rounded-2xl border text-xs font-mono overflow-x-auto"
        style={{
          background: "#0d1117",
          color: "#c9d1d9",
  ...
  if (isPolarType) {
    ...
    return (
      <div
        className="my-6 w-full flex flex-col items-center"
        style={{ height: 350 }}
      >
        {parsed.title && (
          <h4
            className="text-center mb-4 font-bold text-sm"

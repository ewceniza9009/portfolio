    ...
    <div className="flex flex-col" style={{ height }}>
      <div className="flex-1 overflow-auto p-6 space-y-4 text-left">
        {steps.map((step, i) => (
          <div
            key={i}
            className={`transition-all duration-500 ${revealed.includes(i) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 h-0 overflow-hidden"}`}
          >
            <div
              className={`flex items-start gap-3 p-4 rounded-xl border ${i === current ? "border-[var(--accent)] bg-[var(--accent-dim)]" : "border-[var(--border)] bg-[var(--bg-card)]"}`}
            >
              <div
                className={`shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === current ? "bg-[var(--accent)] text-[var(--bg-primary)]" : "bg-[var(--border)] text-[var(--text-secondary)]"}`}
              >
                {i + 1}
              </div>
              <div
      ...
      >
        <button
          onClick={prev}
          disabled={current === 0}
          className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold disabled:opacity-30 transition-all hover:bg-[var(--border)] hover:text-[var(--text-primary)]"
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold disabled:opacity-30 transition-all hover:opacity-90"
  ...
  if (done) {
    ...
    return (
      ...
      >
        <Trophy
          size={40}
          style={{ color: pct >= 70 ? "var(--accent)" : "var(--text-muted)" }}
        />
        <div
...
export default memo(function InteractiveBlock({
  html,
  height = "400px",
  ...
  const renderPreset = () => {
    ...
    if (preset.type === "steps") {
      ...
      const steps = Array.from(div.querySelectorAll("p, li, h3, h4")).map(
        (el) => el.outerHTML,
      );
      return (
        <StepThrough
          steps={steps.length > 0 ? steps : ["Step 1", "Step 2"]}
          height={height}
        />
      );
    }
    ...
    if (preset.type === "quiz") {
      ...
      if (questions.length > 0)
        return <Quiz questions={questions} height={height} />;
  ...
  useEffect(() => {
    ...
    const scripts = Array.from(
      wrapper.querySelectorAll("script:not([data-ib-done])"),
    ) as HTMLScriptElement[];
    scripts.forEach((oldScript) => {
      const rawCode = oldScript.textContent || "";
      const isModule =
        oldScript.type === "module" || rawCode.trim().startsWith("import ");
      ...
      if (isModule) {
        ...
        const safeBody = body.map((line) => {
          ...
          return t.startsWith("[") || t.startsWith("(")
            ? " ".repeat(line.length - t.length) + ";" + t
            : line;
        });
        const patchedBody = safeBody.map((l) =>
          l.replace(
        ...
        const fn = new Function(
          "__ib_container",
          `return (async function(__ib_container){\n${fnBody}\n})(__ib_container)`,
        );
  ...
  return (
    <div
      className="my-6 rounded-xl border overflow-hidden relative"
      style={{
        minHeight: height,

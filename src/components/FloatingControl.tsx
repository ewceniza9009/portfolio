...
const ADVENTURE_STORY: Record<string, AdventureNode> = {
  start: {
    ...
    choices: [
      {
        text: "Basement vector selected. You slip past the loading dock.",
        nextNode: "basement",
      },
      {
        text: "Rooftop vector selected. You climb the fire escape with your node transceiver.",
        nextNode: "rooftop",
      },
      {
        text: "Phishing vector selected. You send an encrypted link disguised as an audit report.",
        nextNode: "phishing",
      },
    ],
  },
  basement: {
    ...
    choices: [
      {
        text: "AC unit overloaded! The guard leaves to investigate. You plug into the port.",
        nextNode: "success",
      },
      {
        text: "You hid behind the rack, but your custom cursor glow reflected on the floor. Caught!",
        nextNode: "fail",
      },
      {
        text: "Reader hacked! But it triggered a silent alarm. Lockdown engaged!",
        nextNode: "fail",
      },
    ],
  },
  rooftop: {
    ...
    choices: [
      {
        text: "Drone jammed! It spins off-course. You finish downloading files.",
        nextNode: "success",
      },
      {
        text: "Vent shaft entry. You escaped, but your connection equipment was seized.",
        nextNode: "fail",
      },
      {
        text: "Telemetry hijacked! You steer the drone away and extract the databases.",
        nextNode: "success",
      },
    ],
  },
  phishing: {
    ...
    choices: [
      {
        text: "Payload injected! Security filters bypassed. Administrative access obtained.",
        nextNode: "success",
      },
      {
        text: "Scrubbed logs. Safe, but you gained no intelligence.",
        nextNode: "fail",
      },
      {
        text: "Disguise call worked! You talked them into verifying the OTP code. Shell active.",
        nextNode: "success",
      },
    ],
  },
...
function MatrixOverlay({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const accentColor =
      rootStyle.getPropertyValue("--accent").trim() || "#a1781b";
    const draw = () => {
...
function SnakeOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-[#0d1117] flex flex-col">
      ...
      >
        <span>
          Snake Game - Score: <strong className="text-white">{score}</strong>{" "}
      ...
      <div className="flex-grow flex items-center justify-center relative p-4">
        <canvas
          ref={canvasRef}
          className="border shadow-lg bg-black"
        ...
        {gameOver && (
          <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-center p-6 z-20">
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={onClose}
                className="px-4 py-2 rounded-xl text-xs font-semibold bg-transparent text-white border hover:bg-white/10"
                style={{ borderColor: "#30363d" }}
              >
                Close
              </button>
...
function TerminalWindow({ onClose }: { onClose: () => void }) {
  ...
  const fileSystem: Record<string, string> = {
    "about.txt":
      "Erwin Wilson Ceniza | Principal Full-Stack Developer | Building commercial web applications and AI integrations.",
    "skills.txt":
      "Frontend: React, TypeScript, Tailwind | Backend: C#, ASP.NET Core, Node.js | Database: SQL Server, PostgreSQL",
    "contact.txt":
      "Email: erwinwilsonceniza@gmail.com | Phone: +63 935-122-8470",
    "secret.md":
      "CLASSIFIED: Building the next generation of web applications...",
  };
  ...
  const handleCommand = (e: React.FormEvent) => {
    ...
    if (activeMode === "guess") {
      const newHistory = [
        ...history,
        { type: "command" as const, content: input },
      ];
      ...
      if (isNaN(guessNum)) {
        newHistory.push({
          type: "error",
          content: 'Please enter a valid number, or type "exit" to quit.',
        });
      } else {
        ...
        if (guessNum === guessTarget) {
          newHistory.push({
            type: "response",
            content: `🎉 CORRECT! You guessed the number ${guessTarget} in ${newAttempts} attempts!`,
          });
          setActiveMode(null);
        } else {
          newHistory.push({
            type: "response",
            content: `${guessNum > guessTarget ? "Too HIGH 👇" : "Too LOW 👆"}. Try again (type "exit" to quit):`,
          });
        }
      }
      setHistory(newHistory);
    ...
    if (activeMode === "adventure") {
      ...
      if (cmd === "exit" || cmd === "quit") {
        ...
        newHistory.push({
          type: "response",
          content: "Exited Cyberpunk adventure.",
        });
        setHistory(newHistory);
        setInput("");
        return;
      ...
      if (adventureNode === "fail" && cmd === "try") {
        ...
        newHistory.push(
          { type: "response", content: "Restarting Netrunner link..." },
          { type: "response", content: ADVENTURE_STORY["start"].text },
        );
        setHistory(newHistory);
        setInput("");
        return;
      ...
      if (
        ...
        newHistory.push({
          type: "error",
          content: `Invalid option. Type '1', '2', or '3' (or 'exit' to quit).`,
        });
      } else {
        ...
        newHistory.push({
          type: "response",
          content: ADVENTURE_STORY[selectedChoice.nextNode].text,
        });
      }
      setHistory(newHistory);
      setInput("");
    ...
    switch (cmd) {
      case "help":
        newHistory.push({
          type: "response",
          content:
            "Available commands: about, skills, contact, resume, clear, cls, date, pwd, whoami, su, sudo su, matrix, hack, snake, guess, adventure",
        });
        break;
      case "about":
        newHistory.push({
      ...
      case "resume":
        newHistory.push({
          type: "response",
          content:
            "Opening resume... (Please use the top navigation button to download the actual PDF)",
        });
        break;
      case "whoami":
        newHistory.push({
        ...
        newHistory.push(
          {
            type: "response",
            content:
              "🔢 GUESS THE NUMBER! I am thinking of a number between 1 and 100.",
          },
          {
            type: "response",
            content: 'Enter your guess (type "exit" to quit):',
          },
        );
        break;
      case "adventure":
      case "rpg":
        ...
        newHistory.push(
          { type: "response", content: "🕵️ CYBERPUNK TEXT ADVENTURE" },
          { type: "response", content: ADVENTURE_STORY["start"].text },
        );
        break;
      case "hack":
        newHistory.push({
          type: "response",
          content: "Initializing cybernetic link...",
        });
        const hackSteps = [
      ...
      case "sudo su":
        if (isRoot)
          ...
          newHistory.push({
            type: "response",
            content:
              "Bypassing security protocols... Access granted. Welcome, root.",
          });
        }
        break;
      case "exit":
      ...
      case "rm -rf /":
        if (isRoot) {
          setHistory([
            {
              type: "error",
              content: "CRITICAL SYSTEM FAILURE: Root file system deleted.",
            },
          ]);
          ...
          newHistory.push({
            type: "error",
            content: "rm: cannot remove '/': Permission denied",
          });
        break;
      case "clear":
      case "cls":
        ...
        return;
      case "pwd":
        newHistory.push({ type: "response", content: currentPath });
        break;
      case "ls":
      case "dir":
        newHistory.push({
          type: "response",
          content:
            "about.txt  skills.txt  contact.txt  secret.md  projects/  experience/",
        });
        break;
      default:
        if (cmd.startsWith("cat ")) {
          ...
          if (fileSystem[file])
            newHistory.push({ type: "response", content: fileSystem[file] });
          else
            newHistory.push({
              type: "error",
  ...
  return (
    <motion.div
      ...
      className={`fixed z-[90] shadow-2xl overflow-hidden font-mono text-sm border ${
        isMaximized
          ? "inset-4 rounded-xl"
          : "sm:left-1/2 sm:-translate-x-1/2 sm:bottom-4 sm:max-w-4xl sm:h-[400px] sm:max-h-[70vh] sm:w-[calc(100%-2rem)] sm:top-auto inset-4 rounded-xl"
      }`}
    ...
    >
      ...
      >
          ...
          <div key={i} className="mb-2">
            {line.type === "command" && (
              <div className="flex items-start gap-2">
                <span
                  className={
                    isRoot ? "text-red-400 font-bold" : "text-green-400"
                  }
                >
        ...
        <form onSubmit={handleCommand} className="flex items-center gap-2 mt-2">
          <span
            className={isRoot ? "text-red-400 font-bold" : "text-green-400"}
          >
            {isRoot ? "root@ewc-os:~#" : "guest@ewc-os:~$"}
          </span>
          <input
...
function ChatWindow({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hey! Ask me anything about Erwin — his skills, projects, experience, or blog posts. 👋",
    },
  ]);
  const [input, setInput] = useState("");
  ...
  const parsedContent = useMemo(() => {
    return messages.map((msg) =>
      msg.role === "assistant" ? parseMarkdown(msg.content) : null,
    );
  }, [messages]);
      ...
      .then((blogs) => {
        if (Array.isArray(blogs) && blogs.length) {
          const lines = blogs
            ...
            .map(
              (b: any) =>
                `- "${b.title}" (${b.tags || "general"}${b.summary ? ": " + b.summary.slice(0, 120) : ""})`,
            );
          setBlogContext("\n\nBLOG POSTS:\n" + lines.join("\n"));
        }
      })
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => setIsMaximized(!isMaximized)}
            className="w-9 h-9 rounded-xl flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/10 transition-all active:scale-90"
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl flex items-center justify-center hover:bg-red-500/10 transition-all active:scale-90"
            style={{ color: "var(--text-secondary)" }}
          >
            <X size={17} />
          </button>
      ...
      >
        {messages.map((msg, i) => (
          ...
          >
            ...
            >
              {msg.role === "assistant" ? (
                <div
                  className="prose prose-xs max-w-none [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_li]:mb-0.5 [&_p]:mb-1 [&_strong]:font-semibold [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded-lg [&_code]:text-[11px] [&_code]:bg-black/10 dark:[&_code]:bg-white/10"
                  style={
                    {
            ...
            {msg.role === "user" && (
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, var(--text-muted) 0%, color-mix(in srgb, var(--text-muted) 70%, var(--text-secondary)) 100%)",
                }}
              >
        ...
        {loading && (
          ...
          >
            ...
            <div
              className="px-5 py-3.5 rounded-2xl rounded-bl-md shadow-sm"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <span
                  className="w-2.5 h-2.5 rounded-full bg-current animate-bounce [animation-delay:0.12s]"
                  style={{
                    color: "var(--accent)",
                    boxShadow:
                      "0 0 8px color-mix(in srgb, var(--accent) 50%, transparent)",
                ...
                />
                <span
                  className="w-2.5 h-2.5 rounded-full bg-current animate-bounce [animation-delay:0.24s]"
                  style={{
                    color: "var(--accent)",
                    boxShadow:
                      "0 0 8px color-mix(in srgb, var(--accent) 50%, transparent)",
...
export default function FloatingControl() {
  ...
  const [cursorOn, setCursorOn] = useState(
    () => (getSafeItem("cursor_enabled") ?? "true") === "true",
  );
  ...
  useEffect(() => {
    ...
    document.body.dataset.windowActive =
      mode === "chat" || mode === "terminal" ? "true" : "false";
    return () => {
      document.body.dataset.fabActive = "false";
      document.body.dataset.windowActive = "false";
    };
  }, [mode]);
  ...
  const toggleCursor = useCallback(() => {
    setCursorOn((prev) => {
      ...
      window.dispatchEvent(
        new CustomEvent("cursor-state-changed", { detail: { enabled: next } }),
      );
      return next;
    });
  }, []);
        ...
        {contactItems.map((item) => (
          <div key={item.label} className="relative">
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                handleCopy(item.copyValue, item.setCopied);
              }}
            ...
            >
              <div
                style={{ color: "var(--accent)", width: 20 }}

...
export default memo(function Interactive3DBlock({
  html,
  height = "420px",
  ...
  useEffect(() => {
    ...
    const resolveUrl = (spec: string): string | null => {
      ...
      if (spec.startsWith("http")) {
        ...
        if (
          s.includes("unpkg.com/three") ||
          /@0\./.test(s) ||
          /three@[\d.]/.test(s)
        ) {
          if (s.includes("orbitcontrols"))
            return BASE + "/examples/jsm/controls/OrbitControls";
          if (s.includes("css2dobject"))
            return BASE + "/examples/jsm/renderers/CSS2DObject";
          if (s.includes("css2d"))
            return BASE + "/examples/jsm/renderers/CSS2DRenderer";
          return BASE;
        }
      }
      return null;
    ...
    const runScripts = () => {
      ...
      const scripts = Array.from(
        wrapper.querySelectorAll("script:not([data-i3d-done])"),
      ...
      scripts.forEach((oldScript) => {
        ...
        if (isModule) {
          ...
          if (header.length > 0) {
            ...
            console.log(
              `[i3d ${I3D_VERSION}] executing: ${header.length} imports, ${body.length} body lines`,
            );
            try {
              ...
              p.catch((err: unknown) => {
                if (running)
                  console.error(`[i3d ${I3D_VERSION}] runtime error:`, err);
              });
            } catch (e) {
            ...
            console.warn(
              `[i3d ${I3D_VERSION}] no imports resolved – script may not work`,
            );
          }
          oldScript.remove();
        } else {
    ...
    const ro = new ResizeObserver(() => {
      ...
      console.log(
        `[i3d ${I3D_VERSION}] canvas sized to ${w}x${h}, running scripts...`,
      );
      runScripts();
      ro.disconnect();
    });

...
sequenceDiagram
    ...
    Clinician->>Server: POST Transit Telemetry (lat, lng, bearing, speed)
    Server->>DB: Query Patient Geofence Boundaries
    DB-->>Server: Resolve Target Home Coordinates
    Note over Server: Compute Geodesic Distance (Haversine Formula)
    opt Distance <= 500m (Privacy Radius)
        Note over Server: Obfuscate Coordinates to (0, 0)
    end
    Server->>Navigator: Stream Obfuscated Location Frame
    Navigator->>Navigator: Project Path Vector to Medical Facility
```
...
sequenceDiagram
    ...
    Client->>Client: Optimistic UI Insert (Generate Temp UUID)
    Client->>Hub: WebSocket Broadcast: SendMessagePayload
    Hub->>Database: Persist Message Entity (Set IsSeen = FALSE)
    Database-->>Hub: Acknowledge Database Commit
    Hub->>Client: WebSocket Push: ReceiveMessage (ID Resolved)
    Client->>Client: Resolve UI Identity (Swap Temp ID -> Permanent UUID)
    Hub->>Patient: WebSocket Fan-Out: ReceiveMessage
    Patient->>Hub: WebSocket Invoke: MarkAsSeen
    Hub->>Database: Transactional Update: Set IsSeen = TRUE
    Hub->>Client: WebSocket Fan-Out: MessageSeenReceipt
    Client->>Client: Transition UI State (Render Double-Blue Read Checks)
```
